...
public class Labrat2ProxyIT
    extends Labrat2ITSupport
{
  ...
  private Labrat2Client proxyClient;
  ...
  public static Option[] configureNexus() {
    return NexusPaxExamSupport.options(
        ...
        nexusFeature("org.sonatype.nexus.plugins", "nexus-repository-labrat2")
    );
  }
  ...
  public void unresponsiveRemoteProduces404() throws Exception {
    Server server = Server.withPort(0).serve("/*")
        ...
        .start();
    try {
      proxyRepo = repos.createLabrat2Proxy("labrat2-test-proxy-notfound", server.getUrl().toExternalForm());
      proxyClient = labrat2Client(proxyRepo);
  ...
  @Test
  public void retrieveLabrat2WhenRemoteOffline() throws Exception {
    Server server = Server.withPort(0).serve("/*")
        .withBehaviours(content("Response"))
        .start();
    try {
      proxyRepo = repos.createLabrat2Proxy("labrat2-test-proxy-offline", server.getUrl().toExternalForm());
      proxyClient = labrat2Client(proxyRepo);
